...
def build_country_code_converter(codeinfo):
    ...
    First = codeinfo['plot_codes']
    Second = codeinfo['data_codes']

    for idx in range(len(data)):
        dict1[data[idx][First]] = data[idx][Second]
...
def reconcile_countries_by_code(codeinfo, plot_countries, gdp_countries):
    ...
    """
    code = build_country_code_converter(codeinfo)
    code_low = dict((key.lower(), value.lower()) for key, value in code.items())
    ...
    for key_plot in plot_countries.keys():
        ...
        for key_gdp in gdp_countries.keys():
            try:
                if (code_low[key_plot.lower()] == key_gdp.lower()):
                    dict1[key_plot] = key_gdp
                else:
                    count += 1
