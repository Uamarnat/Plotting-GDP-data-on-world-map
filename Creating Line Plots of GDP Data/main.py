        ...
        try:
            if value != " ":
                if (int(key) <= gdpinfo["max_year"]) and (int(key) >= gdpinfo["min_year"]):
                    plot_tup_list.append((int(key), float(value)))

        except ValueError:
            pass
    return plot_tup_list
...
def build_plot_dict(gdpinfo, country_list):
    ...
    """
    nest_dict = read_csv_as_nested_dict(gdpinfo["gdpfile"], gdpinfo["country_name"], gdpinfo["separator"],
                                       gdpinfo["quote"])
    plot_dict = {}
    gdp_data = {}
    for country in country_list:
        ...
        for year in range(gdpinfo["min_year"], gdpinfo["max_year"]+1):
            if country in nest_dict:
                gdp_data[str(year)] = nest_dict[country][str(year)]
            else:
                gdp_data[str(year)] = " "

        plot_value = build_plot_values(gdpinfo, gdp_data)
        plot_dict[country] = plot_value
    return plot_dict
...
def render_xy_plot(gdpinfo, country_list, plot_file):
    ...
    XY_chart = pygal.XY(xrange=(1960, 2016))
